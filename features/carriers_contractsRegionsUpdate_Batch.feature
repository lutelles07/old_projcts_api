@ready @carriers @carriers_regions_update_batch
Feature: Endpoint Aldebaran Carriers-Contracts-Regions-Update_Batch

#*************************************************************************************************************
# Generating Reports HMTL: cucumber -p <env> -t @carriersRegionsUpdateBatch --format html --out reports/carriers_contracts_regions_update_batch.html
#*************************************************************************************************************
...
######################################################################################
#   PUT: /aldebaran-carriers/carriers/{cnpj}/contracts/{branch}/{contractId}/regions
######################################################################################

  Scenario: PUT Status Code 200 for path '/aldebaran-carriers/carriers/<cnpj>/contracts/<branch>/<contractId>/regions'
    And   format contract for update region-batch entering regionID equal "1"
    When  PUT in carriers-regions-update-batch passing a "valid" authentication
...
###############################################################################
  Scenario: PUT Status Code 422 for path '/aldebaran-carriers/carriers/<cnpj>/contracts/<branch>/<contractId>/regions'
    And   format contract for update region-batch entering regionID equal "1000"
    When  PUT in carriers-regions-update-batch passing a "valid" authentication
...
#----------------------------------------------------#
  Scenario: PUT Status Code 400 for path '/aldebaran-carriers/carriers/<cnpj>/contracts/<branch>/<contractId>/regions'
    And   format contract for update region-batch entering regionID equal "1"
...
#-------------------------------------------------------------------------------------#
  Scenario: PUT Status Code 401 for path '/aldebaran-carriers/carriers/<cnpj>/contracts/<branch>/<contractId>/regions'
    And   format contract for update region-batch entering regionID equal "1"
    When  PUT in carriers-regions-update-batch passing a "invalid_username" authentication
...
#-------------------------------------------------------------------------------------#
  Scenario: PUT Status Code 401 for path '/aldebaran-carriers/carriers/<cnpj>/contracts/<branch>/<contractId>/regions'
    And   format contract for update region-batch entering regionID equal "1"
    When  PUT in carriers-regions-update-batch passing a "empty_username" authentication
    Then  Response code should be equal "401"
    And   Verify if the response contains the field X-tid
    And  I should see the message "Bad credentials"
...
#-------------------------------------------------------------------------------------#
  Scenario: PUT Status Code 401 for path '/aldebaran-carriers/carriers/<cnpj>/contracts/<branch>/<contractId>/regions'
    And   format contract for update region-batch entering regionID equal "1"
    When  PUT in carriers-regions-update-batch passing a "invalid_password" authentication
    Then  Response code should be equal "401"
    And   Verify if the response contains the field X-tid
    And  I should see the message "Bad credentials"
...
#-------------------------------------------------------------------------------------#
  Scenario: PUT Status Code 401 for path '/aldebaran-carriers/carriers/<cnpj>/contracts/<branch>/<contractId>/regions'
    And   format contract for update region-batch entering regionID equal "1"
    When  PUT in carriers-regions-update-batch passing a "empty_password" authentication
    Then  Response code should be equal "401"
    And   Verify if the response contains the field X-tid
    And  I should see the message "Bad credentials"
